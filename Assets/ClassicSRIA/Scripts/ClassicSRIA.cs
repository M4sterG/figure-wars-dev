...
namespace frame8.ScrollRectItemsAdapter.Classic
{
    ...
    public abstract class ClassicSRIA<TViewsHolder> : MonoBehaviour where TViewsHolder : CAbstractViewsHolder
    {
        ...
        private RectOffset _ZeroRectOffset = new RectOffset();
        private int _OneIfHorizontal_ZeroIfVertical, _OneIfVertical_ZeroIfHorizontal;
        private Coroutine _SmoothScrollToCoroutine;
        ...
        private IEnumerator SetContentInsetFromViewportStart(RectTransform.Edge edge, float fromInset, float toInset, float duration, Action onDone)
        {
        ...
        private void ChangeItemsCount(int changeMode, int itemsCount, int indexIfAppendingOrRemoving = -1, bool contentPanelEndEdgeStationary = false)
        {
            float ctInsetBefore;
        ...
        private void RemoveItemsAndUpdateIndices(int index, int count)
        {
            int rem = count;
            while (rem-- > 0)
            {
        ...
        private void AddItemsAndUpdateIndices(int index, int count)
        {
